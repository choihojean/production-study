		...
		>
			<div className="flex items-center">
				...
				<NavigationMenu>
					<NavigationMenuList>
						{menus.map((menu) => (
							<NavigationMenuItem key={menu.name || 'menu-item'}>
								{menu.items ? (
									<>
																			<Link to={menu.to ?? "/"}>
										<NavigationMenuTrigger>{menu.name || 'Menu'}</NavigationMenuTrigger>
									</Link>
										<NavigationMenuContent>
											<ul className="grid w-[600px] font-light gap-3 p-4 grid-cols-1 md:grid-cols-2">
												{menu.items?.map((item) => (
													<NavigationMenuItem
														key={item.name}
														className={cn([
				...
				<div className="flex items-center gap-2">
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild>
							<Avatar>
								<AvatarImage src="https://github.com/hojinchoi.png" />
